...
double u_exact(double xx, double epsilon, double kappa){
    double num, dem;
    num = 1.0 - exp((kappa*xx)/epsilon);
    dem = 1.0 - exp(kappa/epsilon);
    return (1.0/kappa)*(xx - num/dem);
}
...
int main(){
    ...
    for (int kk = first; kk < size; ++kk) {
        ...
        double Pe_h = 0.8;
        double h = 2.0*epsilon*Pe_h/abs(kappa);

        int nel = (b-a)/h; // number of elements
                    ...
                    for (int i = 0; i < nen; i++){
                        Ke[i][j] = Ke[i][j] + K_func(xx, epsilon)*(dshg[i][l]*2.0/h)*(dshg[j][l]*2.0/h)*w[l]*h/2.0
                               // + gamma_func(xx)*shg[i][l]*shg[j][l]*w[l]*h/2
                                + kappa*(dshg[j][l]*2.0/h)*(shg[i][l])*w[l]*h/2.0;
                    }
        ...
        csvFile << endl;
        for (int i = 0; i < np; ++i) {
            csvFile << u_exact(xl[i], epsilon, kappa);
            if (i < np - 1) {
                csvFile << ","; // Use a comma as a delimiter
            }
        }
