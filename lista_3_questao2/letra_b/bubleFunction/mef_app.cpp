...
double f(double xx){
    return 10.0*exp(-5.0*xx)-4.0*exp(-xx);
}

double u_exact(double xx, double epsilon, double kappa){
    return -1.90476*exp(-5.0*xx)+3.9604*exp(-xx)+5.99498*exp(100*xx)/pow(10,44)-2.05563;
}
...
int main(){
    ...
    while (beta <= 1.0) {
        ...
        // double beta_otm = 1.0 / tanh(Pe_h) - 1.0/Pe_h;
        // beta = beta_otm;
